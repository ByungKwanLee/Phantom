...
# SEED-2-Plus
SEED_2_PLUS_TYPES = ["chart", "map", "web"]
                ...
                for img_items in chunk_lines:
                    ...
                    if len(img_items) != 2:
                        ...
                        for img_item in img_items:
                            ...
                            unique_string_set.update({"-----".join([img_name,question,gt_ans]):pred_ans})
                        for key, value in unique_string_set.items():
                            img_name, question, gt_ans = key.split("-----")
                            new_img_items.append("\t".join([img_name, question, gt_ans, value]))
                        img_items = new_img_items
                    img_correct_num = 0
...
def create_blink_prompt(sub_task, prompt):
    ...
    if sub_task in ['Forensic_Detection', 'Jigsaw', 'Art_Style']:
...
def eval_action_prediction(preds, golds, **kwargs):
    ...
    for pred, gold in zip(preds, golds):
        cur_pred = parse_multi_choice_response_VWB(pred, [chr(ord('A')+i) for i in range(8)])
        try:
...
# ----------- Process Multi-choice -------------
def parse_multi_choice_response_VWB(response: str, all_choices):
    """
    Parse the prediction from the generated response.
    Return the predicted index e.g., A, B, C, D.
    """
